//! Post‑quantum identity using Kyber‑1024 (KEM) and Dilithium‑5 (signatures).
//! Secrets are zeroized on drop.

use zeroize::{Zeroize, ZeroizeOnDrop};
use pqcrypto_kyber::kyber1024;
use pqcrypto_dilithium::dilithium5;

#[derive(Zeroize, ZeroizeOnDrop)]
pub struct SecureEnclave<T> {
    data: Vec<u8>,
    _marker: std::marker::PhantomData<T>,
}

pub struct QuantumIdentity {
    pub id: String,
    kem_public: kyber1024::PublicKey,
    kem_secret: kyber1024::SecretKey,
    sign_public: dilithium5::PublicKey,
    sign_secret: dilithium5::SecretKey,
}

impl QuantumIdentity {
    /// Generate a fresh post‑quantum identity with Kyber‑1024 and Dilithium‑5.
    pub fn new_secure(id: impl Into<String>) -> Self {
        let (kem_pk, kem_sk) = kyber1024::keypair();
        let (sign_pk, sign_sk) = dilithium5::keypair();
        Self {
            id: id.into(),
            kem_public: kem_pk,
            kem_secret: kem_sk,
            sign_public: sign_pk,
            sign_secret: sign_sk,
        }
    }

    /// Encapsulate a shared secret for a recipient using their public key.
    pub fn encapsulate(&self, recipient_public: &kyber1024::PublicKey) -> (Vec<u8>, Vec<u8>) {
        let (ciphertext, shared_secret) = kyber1024::encapsulate(recipient_public);
        (ciphertext.as_bytes().to_vec(), shared_secret.as_bytes().to_vec())
    }

    /// Decapsulate a shared secret using our own secret key.
    pub fn decapsulate(&self, ciphertext: &[u8]) -> Option<Vec<u8>> {
        if ciphertext.len() != kyber1024::ciphertext_bytes() {
            return None;
        }
        let ct = kyber1024::Ciphertext::from_bytes(ciphertext);
        let shared_secret = kyber1024::decapsulate(&ct, &self.kem_secret);
        Some(shared_secret.as_bytes().to_vec())
    }
}
